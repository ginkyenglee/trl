...
from trl.rewards.puzzle_rewards import reward_with_openai_judge_hallucination, reward_with_openai_judge_format
...
def main(script_args, training_args, model_args, dataset_args):
    ...
    train_dataset = load_dataset("puzzleaihf/puzzle-grpo", split="train") \
                                .filter(lambda x: x['hallucination_only'] < 10) \
                                .filter(lambda x: x['task'] in ['concise','descriptive','grammar','structured_summarize']) \
    ...
    train_dataset = load_dataset("puzzleaihf/puzzle-grpo", split="train") \
                                .filter(lambda x: x['hallucination_only'] < 10) \
                                .filter(lambda x: x['format_alignment'] < 10) \
                                .filter(lambda x: x['accuracy'] < 10) \
                                .filter(lambda x: x['overall_score'] < 8) \
                                .shuffle(seed=42).select(range(2000))


    valid_dataset = load_dataset("puzzleaihf/puzzle-grpo", split="valid") \
                                .filter(lambda x: x['hallucination_only'] < 9) \
                                .shuffle(seed=42)
    ...
    trainer = GRPOTrainer(
        ...
        eval_dataset=valid_dataset, #dataset[script_args.dataset_test_split] if training_args.eval_strategy != "no" else None,
